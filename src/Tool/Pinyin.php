...
class Pinyin
{
     ...
     * @return array
     */
    static function get_all_py($chinese, $charset = 'utf-8')
    {
        if ($charset != 'gb2312') $chinese =  self::_U2_Utf8_Gb($chinese);
        $py = self::zh_to_pys($chinese);
     ...
     * @return string
     */
    static function get_first_py($all_pys)
    {
        if (count($all_pys) <= 0) {
            return '';
        }
     ...
     * @return string
     */
    static function get_first_letter($all_pys)
    {
        if (count($all_pys) <= 0) {
            return '';
        }
    ...
    static function zh_to_py($num, $blank = '')
    {
        if ($num > 0 && $num < 160) {
            ...
            foreach (self::$dict_list as $py => $code) {
                if ($code > $num) break;
                $result = $py;
            }
            return $result;
    ...
    static function zh_to_pys($chinese)
    {
        $result = array();
